  ...
  try {
    ...
    res.render('server', {
      ...
      guild: {
        ...
        autoMod: gs.autoMod || { enabled: false, words: [], allowedWords: [], regexWords: [], blockLinks: false, phishingProtection: false, aiFilter: false, logChannelId: '', punishment: 'delete', timeoutMinutes: 10, logLevel: 'all' },
...
app.post('/api/settings/:guildId/automod', requireAuth, requireTopAdmin, (req, res) => {
  ...
  const allowedWords = req.body.allowedWords ? req.body.allowedWords.split(',').map(w => w.trim()).filter(Boolean) : [];
